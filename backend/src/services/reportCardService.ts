...
export class ReportCardService {
  ...
  async getReportCardData(studentId: string, term: string, year: number): Promise<any> {
    ...
    const marksData = marks.map(m => ({
      ...
      subjectCode: (m.subjectId as any)?.code || 'N/A',
  ...
  async generateBulkReportCards(classId: string, term: string, year: number): Promise<Buffer> {
    try {
      ...
      for (const student of students) {
        try {
          ...
          zip.file(`error-${student._id}.txt`, `Failed to generate report: ${(error as Error).message}`);
        }
      }
  ...
  async emailReportCard(studentId: string, term: string, year: number, parentEmail: string): Promise<void> {
  ...
  // Generate PDF content
  private generatePDFContent(doc: PDFKit.PDFDocument, student: any, marks: any[], performance: any, term: string, year: number) {
    try {
      // School Header
      doc.fontSize(22)
         .font('Helvetica-Bold')
      ...
      marks.forEach((mark, index) => {
        const subject = (mark.subjectId as any)?.name || 'Unknown';
