...
const ParentMessages: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('receive_message', (msg: Message) => { setMessages(prev => [...prev, msg]); bumpContact(msg.senderId); });
    socket.on('message_sent', (msg: Message) => { setMessages(prev => [...prev, msg]); bumpContact(msg.receiverId); });
  ...
  const openChat = async (contact: Contact) => {
    ...
    setMsgLoading(true);
    try {
      const r = await fetch(`/api/chats/conversation/${contact._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const sendMessage = () => {
    ...
    socketRef.current.emit('send_message', { senderId: myUserId, receiverId: activeContact._id, message: inputText.trim() });
    setInputText('');
  };
        ...
        <div className="d-flex flex-grow-1 overflow-hidden">
          ...
          <div className="bg-white border-end d-flex flex-column" style={{ width: '280px', minWidth: '280px' }}>
            <div className="p-3 border-bottom">
              <span className="smallest fw-bold text-muted text-uppercase ls-1">Teachers</span>
            </div>
          ...
          <div className="flex-grow-1 d-flex flex-column overflow-hidden">
            {!activeContact ? (
              <div className="flex-grow-1 d-flex align-items-center justify-content-center flex-column gap-2">
                ...
                <span className="smallest fw-bold text-muted text-uppercase ls-1">Select a teacher to start chatting</span>
              </div>
            ) : (
              <>
